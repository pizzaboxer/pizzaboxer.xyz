...
ssh -t $MACHINE "cd /var/www/$DOMAIN; sudo systemctl stop $SERVICE; rm -r $REMOTEDIR; mv $SUBDOMAIN.staging $SUBDOMAIN; sudo systemctl start $SERVICE; systemctl status $SERVICE;"